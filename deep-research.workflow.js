// ============================================================================
// deep-research - complete script for Claude Code's built-in workflow
// (verbatim copy; only this header comment was added)
// ----------------------------------------------------------------------------
// Source: the script automatically written to disk during this run:
//   ~/.claude/projects/-Users-dynamic-pi-workflow/<session>/workflows/scripts/
//     deep-research-wf_432fa07b-d28.js
// Purpose: reading reference. This is a built-in workflow, not a handwritten
// example.
//
// Quick reading guide:
//   - The meta block is pure literal data; its phases correspond one-to-one
//     with the phase() calls below.
//   - 6 JSON Schemas + agent({schema}) means downstream code is deterministic
//     JavaScript, with no string parsing.
//   - pipeline (no barrier) is used for search -> fetch; parallel (barrier) is
//     used before verify to assemble the claim pool.
//   - The comment around line ~246, "Barrier here is intentional", is a concrete
//     example of when to use a barrier.
//   - Starting around line ~248: 3-vote adversarial verification (2/3 refutes
//     kills a claim), with prompts explicitly asking verifiers to REFUTE.
//   - Frequent .filter(Boolean) and abstention handling are the details that
//     keep the run from drifting off course.
// ============================================================================
